...
function displayRuntimeError(){
    var box = $('#runtime-error'), args = Arr(arguments);
    
    box.innerHTML = ""
    if (!args.length) return;
    
    box.innerHTML = '<h6>' + i18n('runtimeErrorTitle') + '</h6>';
    args.forEach(function(a){box.appendChild( $create('p',{},i18n(a)) )})
...
function loadContent () {
    ...
    try {
        ...
        var urlParts = $create('a', {href: sourceUrl});
        ...
        document.body.insertBefore($create('iframe', {src: urlParts.href,frameborder: '0'}), $('#wrapper'));
        ...
        W.addEventListener('beforeunload', function (e) {
            ...
            SA.setSessionTime(sessionEnd - sessionStart);
            SA.LMSCommit("");
            SA.LMSFinish("");
        });
